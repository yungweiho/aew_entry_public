...
.tp
  //裝所有的文章的東西
  .tpBlock(v-for="item in showPost")
    .tpTop
      //標題區
      .tpLeft
        .tpTitle {{ item.title }}
        .tpSubTitle {{ item.domain }} | {{ item.author }}
        .tpPro {{ item.pro }}
        //檔案
        a.tpFileBlock(:href="item.file" target="_blank")
          box
      //封面照
      .tpPic(:style="{background:　'url(' + item.firstpic + ')', backgroundSize: 'contain', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}")
    //文字內容
    .toContent(v-html="item.content")
    
    //TAG
    .TpTagBlock
      .TpTag(v-for="tag in item.hashtag") # {{ tag }}
</template>

<script>
import box from '@/views/Teach/box'
export default {
  components: {box},
  data() {
    return {
  ...
  computed: {
    showPost() {
      var t = this.$route.params.postid
      return this.teachArray.filter(function(item) {return item.postid==t})
    }
  }
}
</script>
